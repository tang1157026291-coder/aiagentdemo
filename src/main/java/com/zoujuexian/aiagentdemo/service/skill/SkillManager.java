...
public class SkillManager {
    ...
    public void reloadSkills() {
        ...
        loadSkills();
        System.out.println("[Skill] 已重新加载所有技能，共 " + skillDefinitions.size() + " 个");
    }

    public boolean addSkillFromContent(String content, String filename) {
        try {
            SkillDefinition definition = parseSkillContent(content, filename);
            if (definition != null) {
                skillDefinitions.removeIf(s -> s.name().equals(definition.name()));
                skillDefinitions.add(definition);
                System.out.println("[Skill] 已添加技能: " + definition.name());
                return true;
            }
        } catch (Exception e) {
            System.err.println("[Skill] 添加技能失败: " + e.getMessage());
        }
        return false;
    }

    public boolean removeSkill(String name) {
        boolean removed = skillDefinitions.removeIf(s -> s.name().equals(name));
        if (removed) {
            System.out.println("[Skill] 已移除技能: " + name);
        }
        return removed;
    }

    private SkillDefinition parseSkillContent(String content, String filename) {
        if (!content.startsWith("---")) {
            System.err.println("[Skill] 文件格式错误（缺少 front matter）: " + filename);
            return null;
        }

        int endIndex = content.indexOf("---", 3);
        if (endIndex < 0) {
            System.err.println("[Skill] 文件格式错误（front matter 未闭合）: " + filename);
            return null;
        }
        ...
        if (name == null || description == null || promptTemplate.isEmpty()) {
            System.err.println("[Skill] 文件缺少必要字段（name/description/prompt）: " + filename);
            return null;
        }
